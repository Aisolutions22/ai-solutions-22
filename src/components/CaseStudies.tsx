...
export function CaseStudies() {
  ...
  const preview = getCases(locale).slice(0, 3);
    ...
    <section id="cases" className="py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-foreground">{d.cases.h2}</h2>
          <p className="mt-4 text-muted-foreground">{d.cases.desc}</p>
        </div>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {preview.map((c) => (
            ...
            >
              ...
              <h3 className="mt-6 font-display font-semibold text-lg text-foreground leading-snug">{c.title}</h3>
              <p className="mt-2 text-sm text-muted-foreground leading-relaxed line-clamp-3">{c.summary}</p>
              <div className="mt-5 flex flex-wrap gap-1.5">
                {c.stack.map((t) => (
                  <span key={t} className="font-mono text-[10px] px-2 py-1 rounded-md bg-foreground/5 text-foreground">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <div className="mt-10 flex justify-center">
          <a
            href={localizeHref("/case-studies", locale)}
